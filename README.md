...
Spring Boot 自带定时Scheduled增加分布式/集群环境下任务调度控制插件，其原理是对任务加锁实现控制，支持能实现分布锁的中间件。
...
2. 方式二：下载本项目的[jar包]()，添加到项目的libs包库中。
3. 方式三：下载本项目的[jar包]()，添加到本地maven库中，然后在pom.xml文件添加引用
...
然后优先级低的服务器则不执行任务。如果优先级高级服务器挂了，中间件不再接收到信息了，优先级低就会接替继续工作。
